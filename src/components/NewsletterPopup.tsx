...
export default function NewsletterPopup() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!EMAIL_RE.test(email)) {
      setError(isRu ? 'Введите корректный email.' : 'Please enter a valid email address.')
      return
    }
        ...
        {!submitted ? (
          <>
            ...
            <form onSubmit={handleSubmit}>
              <input
                ...
                placeholder={isRu ? 'Введите ваш email' : 'Enter your email address'}
              ...
              >
                {isRu ? 'Получить бесплатно' : 'Get Free Access'}
              </button>
            </form>

            <p className="mt-3 text-center text-xs" style={{ color: '#94a3b8' }}>
              {isRu ? 'Без спама. Отписка в любой момент.' : 'No spam, ever. Unsubscribe anytime.'}
            </p>
          </>
        ) : (
          <div className="text-center">
            ...
            <h3 className="mb-2 font-black" style={{ fontSize: 22, color: '#0f172a' }}>
              {isRu ? 'Готово! Проверьте вашу почту.' : "You're in! Check your inbox."}
            </h3>
            <p className="mb-6 text-sm" style={{ color: '#64748b' }}>
              {isRu
                ? 'Материалы уже отправлены. Пока ждёте — попробуйте брокера #1:'
                : 'Your cheat sheet is on its way. While you wait, start trading with our #1 rated broker:'
              }
            </p>
            <a
              href={isRu ? '/pocket/go-rus' : '/pocket/go-en'}
              target="_blank"
            ...
            >
              {isRu ? 'Попробовать Pocket Option бесплатно' : 'Try Pocket Option Free'}
